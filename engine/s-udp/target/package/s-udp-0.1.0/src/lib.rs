...
pub enum AuthMode {
    NoAuth,
    Tokens { peer_token: String, serv_token: String },
    MicroCA { key: [u8; 32] },
}

enum SessionAuth {
    NoAuth,
    Tokens { peer: TokenGuard, server: TokenGuard },
    MicroCA { key: [u8; 32] },
}
...
pub struct Engine {
    ...
    identity: Arc<RwLock<Option<SessionAuth>>>,
...
impl Engine {
    ...
    pub async fn listen(&self, addr: &str, auth: AuthMode) -> Result<mpsc::Receiver<Event>> {
        let socket = Arc::new(UdpSocket::bind(addr).await?);
        
        {
            let mut id = self.identity.write().await;
            *id = Some(match auth {
                AuthMode::NoAuth => SessionAuth::NoAuth,
                AuthMode::Tokens { peer_token, serv_token } => SessionAuth::Tokens {
                    peer: TokenGuard::new(peer_token.into_bytes()),
                    server: TokenGuard::new(serv_token.into_bytes()),
                },
                AuthMode::MicroCA { key } => SessionAuth::MicroCA { key },
            });
        }
    ...
    pub async fn connect(&self, addr: &str, src_port: u16, auth: AuthMode) -> Result<mpsc::Receiver<Event>> {
        ...
        let plaintext = match self.identity.read().await.as_ref() {
            Some(SessionAuth::NoAuth) => Vec::new(),
            Some(SessionAuth::Tokens { peer, .. }) => peer.reveal(),
            Some(SessionAuth::MicroCA { .. }) => Vec::new(),
            None => return Err(anyhow::anyhow!("Identity missing")),
        };
        ...
        for _ in 0..3 {
            ...
            if let Ok(Ok((len, peer_addr))) = tokio::time::timeout(dyn_rto, socket.recv_from(&mut buf)).await {
                if peer_addr == target_addr && len >= 9 && buf[0] == SUDP_AUTH_RESP {
                    let recv_seq = u64::from_be_bytes(buf[1..9].try_into().unwrap_or([0u8; 8]));
                    if recv_seq == SUDP_DIR_BIT { // Server response: all zeros, bit 63 = 1
                        if let Some(decrypted) = self.decrypt_payload(&key, SUDP_DIR_BIT, &buf[0..9], &buf[9..len]) {
                            match self.identity.read().await.as_ref() {
                                Some(SessionAuth::NoAuth) => {
                                    if decrypted.is_empty() { auth_ok = true; }
                                },
                                Some(SessionAuth::Tokens { server, .. }) => {
                                    let mut expected = server.reveal();
                                    println!("Client: decrypted={:?}, expected={:?}", decrypted, expected);
                                    if decrypted == expected { auth_ok = true; }
                                    expected.zeroize();
                                },
                                Some(SessionAuth::MicroCA { key }) => {
                                    let public_key = UnparsedPublicKey::new(&ED25519, key);
                                    if public_key.verify(&shared_key, &decrypted).is_ok() {
                                        auth_ok = true;
                                    }
                                },
                                None => auth_ok = false,
                        ...
                        if let Some(shared) = p.shared_secret {
                            ...
                            if let Some(decrypted) = self.decrypt_payload(&key, 0u64, &buf[0..9], &buf[9..len]) { // Nonce 0: reserved for client handshake
                                if !decrypted.is_empty() {
                                    ...
                                    if let Some(auth_guard) = self.identity.read().await.as_ref() {
                                        match auth_guard {
                                            SessionAuth::NoAuth => {
                                                if decrypted.is_empty() { id_match = true; }
                                            },
                                            SessionAuth::Tokens { peer, .. } => {
                                                println!("Server: peer.verify called");
                                                if peer.verify(&decrypted) { id_match = true; }
                                            },
                                            SessionAuth::MicroCA { .. } => {
                                                if decrypted.is_empty() { id_match = true; }
                                            }
                                    ...
                                    if let Some((_, handshake_data)) = self.handshakes.remove(&addr) {
                                        ...
                                        tokio::spawn(async move {
                                            // 1. Determine payload
                                            let mut payload_data = if id_match {
                                                match engine.identity.read().await.as_ref() {
                                                    Some(SessionAuth::NoAuth) => Vec::new(),
                                                    Some(SessionAuth::Tokens { server, .. }) => server.reveal(),
                                                    Some(SessionAuth::MicroCA { key }) => {
                                                        if let Ok(key_pair) = Ed25519KeyPair::from_seed_unchecked(key) {
                                                            let ss = handshake_data.shared_secret.unwrap();
                                                            key_pair.sign(&ss).as_ref().to_vec()
                                                        } else {
                                                            Vec::new()
                                                        }
                                                    },
